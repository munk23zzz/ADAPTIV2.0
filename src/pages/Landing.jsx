import React from 'react';
import Navbar from '../components/Navbar';
import Hero from '../components/Hero';
import Features from '../components/Features';
import HowItWorks from '../components/HowItWorks';
import TrustBlock from '../components/TrustBlock';
import Pricing from '../components/Pricing';
import Testimonials from '../components/Testimonials';
import Footer from '../components/Footer';
import { Link } from 'react-router-dom';

const Landing = ({ isDark, toggleTheme }) => {
  return (
    <>
      <Navbar isDark={isDark} toggleTheme={toggleTheme} />
      
      <main id="main" className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-32 relative z-10">
        <Hero />
        <Features />
        <HowItWorks />
        <TrustBlock />
        <Pricing />
        <Testimonials />
        
        {/* Final CTA */}
        <section className="text-center py-20 relative rounded-3xl overflow-hidden glass-card">
          <div className="absolute inset-0 bg-gradient-to-r from-primary-light/20 to-secondary-light/20 dark:from-primary-dark/20 dark:to-secondary-dark/20 z-0"></div>
          <div className="relative z-10 space-y-6">
            <h2 className="text-3xl md:text-5xl orbitron font-bold">Mulai Belajar Lebih Cerdas Hari Ini.</h2>
            <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
              Mulai perjalanan belajarmu dengan kekuatan AI. Gratis untuk 50 file pertama.
            </p>
            <div className="pt-4">
              <Link to="/register" className="neon-btn text-lg px-8 py-4">
                Daftar Gratis — Mulai Sekarang
              </Link>
            </div>
            <div className="flex flex-wrap justify-center gap-6 mt-8 text-sm text-slate-500 dark:text-slate-400">
              <div className="flex items-center gap-2">
                <span className="material-icons-round text-primary-dark">verified_user</span>
                <span>Data Aman & Enkripsi</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="material-icons-round text-primary-dark">credit_card_off</span>
                <span>Tanpa Kartu Kredit</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="material-icons-round text-primary-dark">bolt</span>
                <span>Setup &lt; 2 Menit</span>
              </div>
            </div>
          </div>
        </section>
      </main>

      <Footer />
    </>
  );
};

export default Landing;
